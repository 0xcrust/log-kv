...
#[test]
fn cli_no_args() {
    Command::cargo_bin(CARGO_BIN_NAME)
        .unwrap()
        .assert()
        .failure();
}
...
fn cli_get_non_existent_key() {
    let temp_dir = TempDir::new_in("logs").unwrap();
    Command::cargo_bin(CARGO_BIN_NAME)
        .unwrap()
        .args(&["get", "key1"])
...
fn cli_rm_non_existent_key() {
    println!("error: {:?}", kvs::KvsError::KeyNotFound);
...
fn cli_get_stored() -> Result<()> {
    println!("cli_get_stored");
    let temp_dir = TempDir::new_in("logs").expect("unable to create temporary working directory");
    println!("temp_dir: {:?}", temp_dir.path());
    println!("after temp_dir creation");

    let mut store = KvStore::open(temp_dir.path())?;
    println!("after kvstore open");
